...
export default function V2() {
  ...
  return (
    <div style={{ background: bg, color: olive, fontFamily: font.body, minHeight: '100vh' }}>
      ...
      <section ref={heroRef} style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 32 }}>
            {siteData.nav.map(n => {
              const isInternal = n.href.startsWith('/') && !n.href.startsWith('#');
              return isInternal ? (
                <Link key={n.label} to={n.href} style={{ fontFamily: font.body, fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: olive, opacity: 0.6, textDecoration: 'none' }}>
                  {n.label}
                </Link>
              ) : (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 24px', position: 'relative', zIndex: 5 }}>
          ...
          <motion.p
            ...
            style={{ fontFamily: font.display, fontStyle: 'italic', fontSize: 'clamp(16px, 2.5vw, 22px)', lineHeight: 1.4, maxWidth: 440, opacity: 0.6, marginTop: 28 }}
          >
            {siteData.hero.headline}
          </motion.p>
          <motion.p
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3 }}
            style={{ fontFamily: font.body, fontSize: 13, lineHeight: 1.7, maxWidth: 420, opacity: 0.45, marginTop: 12 }}
          >
      ...
      {/* ===== SECTIONS ===== */}
      {[
        { ...siteData.sections.brand, img: siteData.sections.brand.imageUrl, reverse: false },
      ...
      {/* ===== 3D SCANNING ===== */}
      <V2ScanningSection font={font} olive={olive} />
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center' }} id="treatments">
        ...
        <h2 style={{ fontFamily: font.display, fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 400, margin: '0 0 16px' }}>Refined Aesthetic Solutions</h2>
        <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6, maxWidth: 560, margin: '0 auto 48px' }}>
        ...
        <div style={{ display: 'flex', gap: 16, overflowX: 'auto', scrollSnapType: 'x mandatory', paddingBottom: 16 }} className="hide-scrollbar">
          {siteData.treatments.map(t => (
            <Link key={t.name} to={`/care/${t.slug}`} style={{ flex: '0 0 auto', width: 'clamp(240px, 65vw, 300px)', scrollSnapAlign: 'start', textDecoration: 'none' }}>
              <div style={{ aspectRatio: '3/4', borderRadius: 12, overflow: 'hidden', position: 'relative' }}>
                <img src={t.img} alt={t.name} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s ease' }} loading="lazy"
      ...
      <section id="booking" style={{ padding: '80px 24px', display: 'flex', justifyContent: 'center' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: 'clamp(32px, 5vw, 48px)', maxWidth: 480, width: '100%', boxShadow: '0 8px 40px rgba(74,90,82,0.08)' }}>
          ...
          <h2 style={{ fontFamily: font.display, fontSize: 32, fontWeight: 400, textAlign: 'center', margin: '0 0 16px' }}>Begin Your Journey</h2>
          <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6, textAlign: 'center', marginBottom: 32 }}>
            {siteData.sections.booking.text}
          </p>
      ...
      <footer style={{ background: olive, color: cream, padding: '48px 32px 24px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 32, marginBottom: 32, flexWrap: 'wrap' }}>
          {siteData.nav.map(n => {
            const isInternal = n.href.startsWith('/') && !n.href.startsWith('#');
            return isInternal ? (
              <Link key={n.label} to={n.href} style={{ color: cream, opacity: 0.6, fontSize: 12, textDecoration: 'none', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{n.label}</Link>
            ) : (
...
/* 3D Scanning Section for V2 */
function V2ScanningSection({ font, olive }: { font: { display: string; body: string }; olive: string }) {
  const ref = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ['start end', 'center start'] });
  const opacity = useTransform(scrollYProgress, [0, 0.6], [0, 1]);
  const y = useTransform(scrollYProgress, [0, 1], [40, 0]);

  const data = siteData.sections.scanning;

  return (
    <div ref={ref} style={{ padding: '80px 24px', maxWidth: 720, margin: '0 auto' }}>
      <motion.div style={{ y, opacity }}>
        <p style={{ fontFamily: font.body, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', opacity: 0.5, marginBottom: 12 }}>{data.eyebrow}</p>
        <h2 style={{ fontFamily: font.display, fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 400, lineHeight: 1.2, margin: '0 0 8px' }}>{data.title}</h2>
        <p style={{ fontFamily: font.display, fontStyle: 'italic', fontSize: 'clamp(1rem, 1.8vw, 1.15rem)', opacity: 0.5, marginBottom: 28 }}>{data.tagline}</p>
        <div style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.7 }}>
          {data.text.map((p, i) => (
            <p key={i} style={{ marginBottom: i < data.text.length - 1 ? 18 : 0 }}>{p}</p>
          ))}
        </div>
      </motion.div>
    </div>
